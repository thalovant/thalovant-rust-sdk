...
fn frame(name: &str) -> Vec<u8> {
    ...
    let case = frames["cases"]
        ...
        .clone();
    STANDARD.decode(case["frame"].as_str().expect("frame")).expect("base64")
}
...
fn a_payload_type_nobody_named_arrives_under_its_number() {
    ...
    let spec = vectors("binary-vectors.json");
    for (wire, name) in spec["unnamed_kind_names"].as_object().expect("unnamed_kind_names") {
...
fn the_reference_encoders_frames_decode_here() {
    ...
    for case in frames["cases"].as_array().expect("cases") {
        ...
        let raw = STANDARD.decode(case["frame"].as_str().expect("frame")).expect("base64");
        let message = decode_hive_binary_frame(&raw).unwrap_or_else(|e| panic!("{name}: {e}"));
        assert_eq!(message.msg_type, "bin", "{name}");
        let binary = message.binary.as_ref().unwrap_or_else(|| panic!("{name}: no binary"));
        assert_eq!(binary.kind, case["expected_kind"].as_str().expect("kind"), "{name}");
        let clip = STANDARD
            .decode(case["expected_payload"].as_str().expect("payload"))
            .expect("base64");
        assert_eq!(binary.data, clip, "{name}: the clip did not survive the decode");
...
fn every_case_the_vectors_describe_decodes_as_it_says() {
    ...
    for case in spec["cases"].as_array().expect("cases") {
        let name = case["name"].as_str().expect("name");
        let message = decode_hive_binary_frame(&frame(name)).unwrap_or_else(|e| panic!("{name}: {e}"));
        let binary = message.binary.as_ref().unwrap_or_else(|| panic!("{name}: no binary"));
        let expected = &case["expected"];
        assert_eq!(binary.kind, expected["kind"].as_str().expect("kind"), "{name}");
